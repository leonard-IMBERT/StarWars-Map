...
export default class Drawer {
  ...
  /**
   * Draw text centered on the specified position
   ...
   */
  drawTextCentered(posX, posY, text, color) {
    const measure = this.ctx.measureText(text);
    this.drawText(posX - measure.width / 2, posY, text, color);
  }
   ...
   * @param {boolean} fill Switch to true to fill the hexagon
   */
  drawHexagon(posx, posy, size, color, fillcolor, fill = false) {
    const fillc = fillcolor || color;
